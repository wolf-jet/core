module.exports = class Events {
  ...
  #OnUpdate = async ({ body }) => {
    const { id } = body;
    if (this.#Client.Groups.cache.has(id)) {
      this.#Client.Groups.cache.delete(id);
      // TODO: add no cache
      const user = await this.#Client.Groups.GetGroup(id, ["base"], true);
      this.#Client.Groups.cache.set(id, user);
    }
    this.Updated(id);
  };
};
