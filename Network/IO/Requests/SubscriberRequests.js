module.exports = class SubscriberRequests {
   ...
   */
  static SubscriberProfile = async (io, id, extended = false, subscribe = false) =>
   ...
   */
  static SubscriberProfiles = async (io, idList, extended = false, subscribe = false) =>
    await io.Emit(
      "subscriber profile",
      {
        headers: { version: 4 },
   ...
   */
  static SubscriberGroupsList = async (io) => await io.Emit("subscriber group list", {});
